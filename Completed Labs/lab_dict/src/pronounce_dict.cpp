...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    std::transform(word1.begin(), word1.end(), one.begin(), toupper);
    std::transform(word2.begin(), word2.end(), two.begin(), toupper);

    len1 = dict.count(one);
    len2 = dict.count(two);

    if (len1 > 0 && len2 > 0 && (dict.at(one) == dict.at(two))) return true;
    return false;
}
