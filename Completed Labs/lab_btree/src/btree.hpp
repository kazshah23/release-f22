...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    //if the index is in bound and already exists then just break out of function
    if (first_larger_idx < subroot->elements.size() && subroot->elements[first_larger_idx] == pair) { return; }

    //Just insert ChildNode into pair.
    BTreeNode* child_node = subroot->children[first_larger_idx];
    insert(child_node, pair);
    //we need to check if the child became too large and thus needs to be split to maintain order.
    //if the size is greater than m then you must split the child and make the child less than equal order
