...
void DHHashTable<K, V>::insert(K const& key, V const& value)
{
     ...
     */
    auto curr = hashes::hash(key, size);
    while (table[curr] != nullptr) curr = (curr + hashes::secondary_hash(key, size)) % size;
...
void DHHashTable<K, V>::remove(K const& key)
{
    ...
    while (table[curr] != nullptr && table[curr]->first != key) {
        curr = (curr + hashes::secondary_hash(key, size)) % size;
    }
...
int DHHashTable<K, V>::findIndex(const K& key) const
{
    ...
    while (should_probe[curr]) {
        if (table[curr] != nullptr) {
            if (table[curr]->first == key) return curr;
        }
        curr = (curr + hashes::secondary_hash(key, size)) % size;
    }
    return -1;
