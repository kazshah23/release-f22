                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    if (singleQueue.size() == 0) {
        output = mergeQueue.front();
        mergeQueue.pop();
        return output;
    }
    if (mergeQueue.size() == 0) {
        output = singleQueue.front();
        singleQueue.pop();
        return output;
    } 
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    while (singleQueue.size() + mergeQueue.size() > 1) {
        ...
        TreeNode* ab = nullptr;
        int freqA = (a->freq).getFrequency();
        int freqB = (b->freq).getFrequency();
        ab = new TreeNode(freqA + freqB);
        ab -> left = a;
        ab -> right = b;
        mergeQueue.push(ab);
    }
...
void HuffmanTree::writeTree(TreeNode* current, BinaryFileWriter& bfile)
{
    ...
    if (current) {
        if (!(current -> right) && !(current -> left)) {
            ...
            bfile.writeBit(0);
            if (current -> left) writeTree(current -> left, bfile);
            if (current -> right) writeTree(current -> right, bfile);
        }
    }
}
