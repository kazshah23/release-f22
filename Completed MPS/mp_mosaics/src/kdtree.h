...
class KDTree
{
  ...
  public:
    ...
    KDTreeNode* buildTree(int left, int right, int dim);
    ...
    void MakeCopy(KDTreeNode* Node, KDTreeNode* NodeToCopy);
    KDTreeNode* copy(const KDTreeNode* other);
    ...
    Point<Dim> find_neighbor(const Point<Dim>& query, int dim, KDTreeNode* subroot) const;
