...
V2D file_to_V2D(const std::string & filename){
    // debugging
        std::string txt = file_to_string(filename);
    ...
    if (file.is_open()) {
        while(getline(file, line)) {
            ...
            int s = SplitString(line, ',', vec);
            if (s == 0) {
                vec[0] = Trim(vec[0]);
                output.push_back(vec);
                return output;
            }
            for (int i = 0; i < s; i++) vec[i] = Trim(vec[i]);
            output.push_back(vec);
        }
    }
    return output;
...
V2D clean(const V2D & cv, const V2D & student) {
    ...
    for (unsigned i = 0; i < cv.size(); i++) {
        ...
        // this is the name of our data variables
        c.push_back(cv[i][0]);
        // these are 2d iterations
        for (unsigned j = 1; j < cv[i].size(); j++) {
            for (unsigned a = 0; a < student.size(); a++) {
                for (unsigned b = 0; b < student[a].size(); b++) {
                    // if matches than push back into the 1d vector c 
                    if (student[a][0] == cv[i][j]) {
                        if (student[a][b] == cv[i][0]) {
                            c.push_back(cv[i][j]);
        ...
        // pushback c into the 2d output 
        if (c.size() > 1) output.push_back(c);
    }
    return output;
}
